...
func (uc *useCase) QueryPublishList(ctx context.Context, req *model.VideoReq) ([]*model.Video, int64, error) {
	exist, err := uc.db.IsVideoExist(ctx, req.Uid)
	if err != nil {
		return nil, 0, fmt.Errorf("Check videoId exist failed: %w", err)
	}
	if !exist {
		return nil, 0, errno.NewErrNo(errno.ServiceVideoNotExist, "Video not exist")
	}
