...
type InteractUseCase interface {
	...
	Comment(ctx context.Context, req *model.InteractReq) (int64, error)
...
func NewInteractUseCase(db repository.InteractDB, svc *service.InteractService, cache repository.InteractCache, rpc repository.RpcPort) *useCase {
	return &useCase{
		db:    db,
		cache: cache,
		svc:   svc,
